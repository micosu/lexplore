...
student_metrics = pd.read_csv("during_study/new_results/june11/engineered_metrics.csv")
...
# Control: students not in metrics OR in metrics but total_playtime < 60
# Experimental: students in metrics with total_playtime >= 60

# Create a dictionary of student IDs to their total_playtime
playtime_dict = dict(zip(student_metrics["service_id"], student_metrics["trained_learned"]))
def assign_group(student_id):
    if student_id not in playtime_dict:
        return "Control"  # Not in metrics = Control
    elif playtime_dict[student_id] < .1:
        return "Control"  # In metrics but playtime < 60 = Control
    else:
        return "Experimental"  # In metrics with playtime >= 60 = Experimental

tests["group"] = tests["id"].apply(assign_group)
